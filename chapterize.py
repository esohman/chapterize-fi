...
class Book():
    ...
    def getHeadings(self):
        ...
        # Form 1: Luku I, Luku 1, LUKU 1, Luku Yksi
        # Ways of enumerating chapters, e.g.
        arabicNumerals = '\d+'
        romanNumerals = '(?=[MDCLXVI])M{0,3}(CM|CD|D?C{0,3})(XC|XL|L?X{0,3})(IX|IV|V?I{0,3})'
        numberWordsByTens = ['kaksikymmentä', 'kolmekymmentä', 'neljäkymmentä', 'viisikymmentä', 'kuusikymmentä',
                              'seitsemänkymmentä', 'kahdeksankymmentä', 'yhdeksänkymmentä']
        numberWords = ['yksi', 'kaksi', 'kolme', 'neljä', 'viisi', 'kuusi',
                       'seitsemän', 'kahdeksan', 'yhdeksän', 'kymmenen', 'yksitoista',
                       'kaksitoista', 'kolmetoista', 'neljätoista', 'viisitoista',
                       'kuusitoista', 'seitsemäntoista', 'kahdeksantoista', 'yhdeksäntoista'] + numberWordsByTens
        numberWordsPat = '(' + '|'.join(numberWords) + ')'
        secondaryOrdinals = ['kolmas', 'neljäs', 'viides', 'kuudes', 'seitsemäs', 'kahdeksas', 'yhdeksäs']
        ordinalNumberWordsByTens = ['kahdeskymmenes', 'kolmaskymmenes', 'neljäskymmenes', 'viideskymmenes', 
                                    'kuudeskymmenes', 'seitsemänkymmenes', 'kahdeksaskymmenes', 'yhdeksäskymmenes'] + \
                                    numberWordsByTens
        ordinalNumberWords = ['ensimmäinen', 'toinen', 'kolmas', 'neljäs', 'viides', 'kuudes', 
                              'seitsemäs', 'kahdeksas', 'yhdeksäs', 'kymmenes', 'yhdestoista','kahdestoista','viimeinen'] + \
                             [secondaryOrdinal + 'toista' for secondaryOrdinal in secondaryOrdinals] + ordinalNumberWordsByTens
        ...
        form1 = 'luku ' + enumerators
        form5 = enumerators + " luku"
